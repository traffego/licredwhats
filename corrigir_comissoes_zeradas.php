...
echo "<title>Correção de Comissões Zeradas</title>";
echo "<style>
...
echo "<h1>Correção de Comissões Zeradas</h1>";

// Verificar se estamos no modo de execução
$executar = isset($_GET['executar']) && $_GET['executar'] == 1;

if (!$executar) {
    echo "<p>Este script irá corrigir as comissões que foram zeradas, aplicando o valor correto em cada uma para que a soma seja exatamente 40% do lucro.</p>";
    echo "<p>O script fará o seguinte:</p>";
    echo "<ol>";
    echo "<li>Identificar todas as comissões com valor zero</li>";
    echo "<li>Agrupar por empréstimo</li>";
    echo "<li>Calcular o valor correto da comissão (40% do lucro)</li>";
    echo "<li>Distribuir os valores entre as parcelas</li>";
    echo "</ol>";
    ...
    try {
        // 1. Identificar empréstimos com comissões zeradas
        $sql = "SELECT 
        ...
        echo "<p>Encontrados $total_emprestimos empréstimos com comissões zeradas.</p>";
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Buscar todas as comissões zeradas deste empréstimo
            ...
            if ($num_comissoes == 0) {
                echo "<p class='warning'>Nenhuma comissão zerada encontrada para o empréstimo #$emprestimo_id. Pulando...</p>";
                continue;
            }
            ...
            echo "<p>Comissão total correta (40% do lucro): R$ " . number_format($comissao_total_correta, 2, ',', '.') . "</p>";
            ...
            echo "<table>";
            echo "<tr><th>ID</th><th>Descrição</th><th>Novo Valor</th></tr>";
            
            // Atualizar as comissões
            for ($i = 0; $i < $num_comissoes; $i++) {
                $comissao_id = $comissoes[$i]['id'];
                $descricao = $comissoes[$i]['descricao'];
